...
//  Enable::INTT_SUPPORT = true; // enable global support structure readout
  ...
  // OFFLINE: Acts geometry (MakeActsGeometry, pulled in by clustering AND tracking)
  // reads alignment from the BNL conditions DB, which is unreachable here -> any
  // se->run() would throw. Disable the whole digitization/reconstruction chain.
  // Geant4 simulation still runs, so the event display shows the simulated
  // particle trajectories/hits in the detector geometry (truth-level view).
  Enable::MVTX_CELL = false;  Enable::MVTX_CLUSTER = false;  Enable::MVTX_QA = false;
  Enable::INTT_CELL = false;  Enable::INTT_CLUSTER = false;  Enable::INTT_QA = false;
  Enable::TPC_CELL  = false;  Enable::TPC_CLUSTER  = false;  Enable::TPC_QA  = false;
  Enable::MICROMEGAS_CELL = false; Enable::MICROMEGAS_CLUSTER = false; Enable::MICROMEGAS_QA = false;
  Enable::TRACKING_TRACK = false; Enable::TRACKING_EVAL = false; Enable::TRACKING_QA = false;
  Enable::GLOBAL_RECO = false; Enable::GLOBAL_FASTSIM = false;
  Enable::CENTRALITY = false;
  Enable::JETS = false; Enable::JETS_EVAL = false;
