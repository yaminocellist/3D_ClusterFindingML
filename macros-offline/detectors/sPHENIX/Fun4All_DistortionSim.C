#ifndef MACRO_FUN4ALL_DISTORTIONSIM_C
#define MACRO_FUN4ALL_DISTORTIONSIM_C
...
// plot_trackers.C - OFFLINE tracker geometry / event display (no conditions DB).
// Reconstructed from the ana.331-era Fun4All_G4_sPHENIX.C. Changes vs. that base:
//   (1) renamed to plot_trackers so `root -l plot_trackers.C` auto-runs it,
//   (2) Enable::DISPLAY = true,
//   (3) an OFFLINE block (just before G4Init) disabling all digitization /
...
// Fun4All_DistortionSim.C - BATCH TPC distortion simulation.
// Geant4 + digitization with the static+module-edge distortion map applied,
// + clustering + SvtxEvaluator -> ntp_hit / ntp_cluster in <out>_g4svtx_eval.root.
// Distortion settings are forced in the body below (self-contained), so this works
// with the CVMFS-default G4_TPC.C - no ROOT_INCLUDE_PATH change needed.
// Pick an ana331-compatible alignment in run_batch_sim.sh (ana494/ana537 alignments
// crash on the ana.331 software). Acts track fitting is opt-in: export ENABLE_TRACKING=1.
int Fun4All_DistortionSim(
    const int nEvents = 1,
...
// again on the DST in the Flags node under the RUN node
  ...
  // batch distortion sim: never open the display
...
//  G4MAGNET::magfield_rescale = 1.;  // make consistent with expected Babar field strength of 1.4T
  ...
  // ===================================================================
  // OFFLINE block (no conditions DB) -- overrides all Enable flags above.
  // Keep detector GEOMETRY on (MVTX/INTT/TPC/MICROMEGAS volumes) so the
  // tracker shows up, but turn OFF every reco step that pulls from the
  // BNL conditions DB (digitization cells, clustering, tracking/Acts
  ...
  if (!use_cdb) {
      // No offline alignment selected -> truth-only (disable DB-dependent reco).
      Enable::QA = false;
      ...
      // Distortion sim: digitization (PHG4TpcElectronDrift applies the distortion ->
      // TRKR_HITSET) + clustering + tracking + SvtxEvaluator. ntp_hit holds the DISTORTED
      // hits; ntp_cluster the clusters. All local-file driven (localAlignmentParamsFile +
      // the distortion-map path below) -- no CDB network.
      // NOTE: tracking MUST be ON -- SvtxEvaluator/SvtxTruthRecoTableEval assert on the
      // SvtxTrackMap, so it aborts if tracking is disabled.
      std::cout << ">>> Distortion sim: digitization + clustering + tracking + eval ON <<<" << std::endl;
      Enable::MVTX_CELL = Enable::MVTX_CLUSTER = true;
      Enable::INTT_CELL = Enable::INTT_CLUSTER = true;
      Enable::TPC_CELL  = Enable::TPC_CLUSTER  = true;
      Enable::MICROMEGAS_CELL = Enable::MICROMEGAS_CLUSTER = true;
  ...
  Enable::HCALIN = Enable::HCALIN_ABSORBER = Enable::HCALIN_CELL = Enable::HCALIN_TOWER =
    Enable::HCALIN_CLUSTER = Enable::HCALIN_EVAL = Enable::HCALIN_QA = false;
  Enable::HCALOUT = Enable::HCALOUT_ABSORBER = Enable::HCALOUT_CELL = Enable::HCALOUT_TOWER =
    Enable::HCALOUT_CLUSTER = Enable::HCALOUT_EVAL = Enable::HCALOUT_QA = false;
  Enable::EPD = false;
  Enable::ZDC = Enable::ZDC_TOWER = Enable::ZDC_EVAL = false;

  // downstream reco that depends on the above / on the DB
  Enable::TOPOCLUSTER = false;
  Enable::PARTICLEFLOW = false;
  Enable::CALOTRIGGER = false;
  Enable::GLOBAL_RECO = false;
  Enable::GLOBAL_FASTSIM = false;
  Enable::CENTRALITY = false;
  Enable::JETS = Enable::JETS_EVAL = Enable::JETS_QA = false;
  Enable::HIJETS = false;
